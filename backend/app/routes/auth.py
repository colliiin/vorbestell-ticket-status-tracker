...
from app.security.rate_limit import check_rest_rate, reset_rest_rate
...
def login(request: Request, payload: LoginIn, response: Response, db: Session = Depends(get_db)):
    rate_bucket = f"login:{payload.username.strip().lower()}"
    check_rest_rate(request, rate_bucket, 5, 60)
    ...
    reset_rest_rate(request, rate_bucket)
