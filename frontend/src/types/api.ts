...
export type PublicTicket = { ticket_number: string; customer_name: string; status: TicketStatus; created_at: string; status_changed_at: string; closed_at?: string | null; items: TicketItem[]; total_price: string };
...
export type StatusChangedData = { ticket_id: number; old_status: TicketStatus; new_status: TicketStatus; message: string; created_at: string };
export type WsEvent = { type: "chat_message"; data: ChatMessage } | { type: "status_changed"; data: StatusChangedData } | { type: "error"; data: { message: string } };
